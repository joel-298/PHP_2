...
if ($query->execute()) {
    ...
    if ($result->num_rows === 1) { // EXISTS
        ...
        if ($row['is_email_verified'] === '1') { // Email verified now send otp
            $query->close();
            // OTP LOGIC : 
            ...
            $MailContent = "✅ Please verify your account : " . $otp . " and then you can change your password !";
            $name = $row['first_name'] . " " . $row['last_name'];
            sendEmail($name, $email, $MailContent);

            $_SESSION["id"] = $id;
            http_response_code(200) ; 
            echo json_encode(["status" => "success", "message" => "Otp Send Please enter otp and then change your password !", 'page' => $_SESSION["page"]]);
            exit;
            ...
            $_SESSION["id"] = $id;
            http_response_code(200) ; 
            echo json_encode(["status" => "error", "message" => "Please verify your email", 'page' => $_SESSION["page"]]);
            exit;
        }

    } else { // USER DOES NOT EXISTS 
        $query->close(); // free up space
        http_response_code(404) ; // not found CORRECT ITS STATUS 
